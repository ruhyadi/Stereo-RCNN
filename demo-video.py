...
def parse_args():
  ...
  parser.add_argument('--left_path', dest='left_path',
                      help='path to left video', default="left",
                      type=str)
  parser.add_argument('--right_path', dest='right_path',
                      help='path to right video', default="right",
                      type=str)
...
if __name__ == '__main__':
  ...
  with torch.no_grad():
    ...
    left_cap = cv2.VideoCapture(args.left_path)
    right_cap = cv2.VideoCapture(args.right_path)
    ...
    while (left_cap.isOpened()):
      ...
      img_left = left_frame.copy()
      img_right = right_frame.copy()
      ...
      im2show_left = np.copy(left_frame)
      im2show_right = np.copy(right_frame)
      ...
      if args.use_lidar:
        ...
        im_box = np.ones((im2show_left.shape[0]*2, im2show_left.shape[0], 3), np.uint8)
